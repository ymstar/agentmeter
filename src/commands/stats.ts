  ...
  try {
    ...
    if (toolStats.length > 0) {
      ...
      for (const stat of toolStats.slice(0, 10)) {
        console.log(
          `  ${stat.tool_name.padEnd(20)} ${String(stat.call_count).padStart(8)} ${formatTokens(stat.total_input_tokens).padStart(10)} ${formatTokens(stat.total_output_tokens).padStart(10)} ${formatCost(stat.total_cost).padStart(10)}`,
        );
      }
    }
...
function printPeriod(label: string, data: { calls: number; input_tokens: number; output_tokens: number; cost: number }): void {
  console.log(`  ${label}:`);
